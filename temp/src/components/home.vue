...
let eventSource = null; // For SSE streaming
let aborter = null;
...
async function send(customText) {
  ...
  try {
    ...
    eventSource = new EventSource(
      `/api/chat/stream?message=${encodeURIComponent(text)}&mode=${mode}`
    );
    ...
    eventSource.onmessage = (event) => {
      ...
      if (chunk === "[DONE]") {
        ...
        trimHistory();
        saveChat();
        const fullText = messages.value.find((m) => m.id === assistantMsgId)?.text || "";
        speak(fullText);
        return;
      }
    ...
    eventSource.onerror = (err) => {
      ...
      error.value = "Feil ved mottak av svar";
      loading.value = false;
